# AGENT.md – Engineering & AI Standards
...
```
.ai/
├── scripts/               # AI-generated utilities
├── docs/                  # AI-generated documentation  
└── archive/               # Temporary experiments (auto-cleanup)
src/
...
✅ Run `npm run lint && npm run type-check` before commits  
✅ Clean `.ai/archive/` after sessions  
✅ **Time Tracking**: Use time tracking commands in every session  
...
- ⏱️ **Session Start**: `npm run time:start` when user provides new task
- 📋 **Planning Phase**: Active during TodoWrite and research/reading
- ⚡ **Execution Phase**: `npm run time:execution` when transitioning from planning to coding
- 🔧 **Rework Phase**: `npm run time:rework` when user indicates errors or fixes needed
- ⏹️ **Session End**: `npm run time:stop` when all todos completed
